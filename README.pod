...
a query string (in the case of a search), or in C<repo/package> syntax.  If
a target is specified as a lone dash (-), additional targets will be read from
stdin.
...
Interpret targets as URLs or paths to local files.
